/**
 * IMPORTS
 */
import { Injectable } from '@nestjs/common';

// services
import { PrismaService } from '../../../infra/database/prisma.service';

// dtos
import { PaginationQueryUsersSystemSettingsDto } from '@src/core/shared/dtos/users-system-settings/pagination-query-create-users-system-settings.dto';

/**
 * Sempre que criar uma classe com Injectable,
 * ela deve ser registrada no module (providers)
 */
@Injectable()
export class GetAllUsersSystemSettingsService {
  constructor(private prisma: PrismaService) {}

  async findAll({
    page = 1,
    limit = 10,
  }: PaginationQueryUsersSystemSettingsDto) {
    const skip = (page - 1) * limit;

    const [data, total] = await Promise.all([
      this.prisma.usersSystemSettings.findMany({
        skip,
        take: limit,
        orderBy: { created_at: 'desc' },
      }),
      this.prisma.usersSystemSettings.count(),
    ]);

    return {
      total,
      totalPages: Math.ceil(total / limit),
      page,
      limit,
      data,
    };
  }
}
